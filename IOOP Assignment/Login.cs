                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            if (reader.HasRows)
                            {
                                ...
                                if (role.ToLower() == "admin")
                                {
                                    ...
                                    adminForm.SetName(reader["username"].ToString());
                                    adminForm.ShowDialog();
                                }
                                else if (role.ToLower() == "customer")
                                {
                                    Customer customerForm = new Customer();
                                    customerForm.SetName(reader["username"].ToString());
                                    customerForm.ShowDialog();
                                }
                                else if (role.ToLower() == "chef")
                                {
                                    Chef chefForm = new Chef();
                                    chefForm.SetName(reader["username"].ToString());
                                    chefForm.ShowDialog();
                                }
                                else if (role.ToLower() == "manager")
                                {
                                    Manager managerForm = new Manager();
                                    managerForm.SetName(reader["username"].ToString());
